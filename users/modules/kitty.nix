...
with lib; let
  ...
  listToFontFeaturesString = list:
    concatStringsSep " " (map (feat: "${optionalString ((substring 0 1 feat) != "-") "+"}${feat}") list);

  attrsToKittyFontFeatures = attrs:
    concatStringsSep "\n" (
      mapAttrsToList (option: value: "font_features ${option} ${listToFontFeaturesString value}") attrs
    );
